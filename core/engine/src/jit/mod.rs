...
mod tests {
    ...
    #[test]
    fn native_admission_preserves_observed_closure_affine_summaries() {
        let code = first_function_code(
            "const N = 100;\n\
             function main() {\n\
                 let accumulator = 0;\n\
                 for (let index = 0; index < N; index++) {\n\
                     accumulator = (accumulator + add(index)) | 0;\n\
                 }\n\
                 return accumulator;\n\
             }\n\
        ...
        assert!(
            InstructionIterator::new(&code.bytecode).any(|(_, _, instruction)| {
                matches!(instruction, Instruction::PureClosureAffineLoopIteration)
            }),
            "the bytecompiler must mark the captured-affine candidate before JIT admission"
        );
        assert!(
            native::admission_profile(&code, false).is_ok(),
            "an unproven captured-affine candidate must retain ordinary native lowering"
        );

        code.mark_pure_range_loop_observed();
        let rejection = native::admission_profile(&code, false)
            .expect_err("native lowering must not discard an observed closure range summary");
        assert_eq!(rejection.kind, JitCompileBlockerKind::UnsupportedOpcode);
        assert_eq!(
            rejection.first_blocking_opcode,
            Some(Opcode::PureClosureAffineLoopIteration)
        );
    }
    ...
    #[test]
    fn context_owned_jit_preserves_named_floating_nan() {
        let mut context = Context::default();
        context.enable_jit();
        let script = crate::Script::parse(
            crate::Source::from_bytes(
                "function read(object, n) { let result = 0.5; for (let i = 0; i < n; i++) { result = object.value; } return result; } let object = { value: NaN }; let answer = 0; for (let j = 0; j < 80; j++) { answer = read(object, 3); } answer !== answer",
            ),
            None,
            &mut context,
        )
    ...
    #[test]
    fn context_owned_jit_deopts_named_floating_load_on_shape_mismatch() {
        let mut context = Context::default();
        context.enable_jit();
        let setup = crate::Script::parse(
            crate::Source::from_bytes(
                "function readFloat(object, n) { let result = 0.5; for (let i = 0; i < n; i++) { result = object.value; } return result; } let floatObject = { value: 1.25 }; let warm = 0; for (let j = 0; j < 80; j++) { warm = readFloat(floatObject, 3); } warm",
            ),
            None,
            &mut context,
        )
        ...
        let changed = crate::Script::parse(
            crate::Source::from_bytes("floatObject.extra = 1; readFloat(floatObject, 3)"),
            None,
            &mut context,
        )
        .expect("parse changed shape");
        let result = changed
            .evaluate(&mut context)
            .expect("evaluate changed shape");
        assert_eq!(result.as_number(), Some(1.25));
    ...
    /// Floating named-load counterpart to `jit_loop_perf`. Run with:
    /// `cargo test -p boa_engine --features jit --release jit_named_f64_load_perf -- --ignored --nocapture`
    #[test]
    #[ignore = "perf measurement; run manually with --release --nocapture"]
    fn jit_named_f64_load_perf() {
        ...
        let time = |jit: bool| -> (f64, std::time::Duration, Option<JitStats>) {
            ...
            drop(
                script
                    .evaluate(&mut context)
                    .expect("warm named floating-load benchmark"),
            );
            let start = Instant::now();
            let value = script
                .evaluate(&mut context)
                .expect("measure named floating-load benchmark");
            (
                value.as_number().unwrap_or_default(),
                start.elapsed(),
                context.jit_stats(),
        ...
        let (interpreter_value, interpreter_time, _) = time(false);
        let (jit_value, jit_time, stats) = time(true);
        assert_eq!(interpreter_value.to_bits(), 499_250_500.5_f64.to_bits());
        assert_eq!(jit_value.to_bits(), interpreter_value.to_bits());
        eprintln!(
            "jit_named_f64_load_perf: interpreter={interpreter_time:?} jit={jit_time:?} ratio={:.3} stats={stats:?}",
            jit_time.as_secs_f64() / interpreter_time.as_secs_f64()
        );
    }
