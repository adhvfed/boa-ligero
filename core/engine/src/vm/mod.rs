...
pub(crate) use {
    ...
    inline_cache::{ElementIC, IndexedKind, InlineCache},
};
...
pub struct Vm {
    ...
    #[cfg(test)]
    pub(crate) pure_affine_loop_reductions: u64,
    ...
    #[cfg(test)]
    pub(crate) pure_method_loop_reductions: u64,
    ...
    #[cfg(test)]
    pub(crate) pure_indexed_reader_loop_reductions: u64,
...
impl Context {
    ...
    /// Checks that `count` native iterations fit within the current frame's
    /// remaining loop limit without consuming them.
    #[inline]
    pub(crate) fn check_loop_iterations(&self, count: u64) -> JsResult<()> {
        let max = self.vm.runtime_limits.loop_iteration_limit();
        let previous_iteration_count = self.vm.frame().loop_iteration_count;
    ...
    pub(crate) fn consume_loop_iterations(&mut self, count: u64) -> JsResult<()> {
        self.check_loop_iterations(count)?;

        let frame = self.vm.frame_mut();
        frame.loop_iteration_count = frame.loop_iteration_count.wrapping_add(count);
        Ok(())
    }
    ...
    /// Run the current frame using only the bytecode interpreter.
    pub(crate) fn run_interpreter(&mut self) -> CompletionRecord {
        while let Some(byte) = self
            .vm
            .frame()
            .code_block
    ...
    fn run_with_jit_backend(&mut self, backend: &mut crate::jit::JitBackend) -> JitRunOutcome {
        'scheduler: loop {
            ...
            if self.vm.frame().jit_entry_attempted() {
                let frame_depth = self.vm.frames.len();
            ...
            // This frame's entry decision is still open. Keep control until a
            // frame transition, a backward edge makes it hot, or a backward
            // edge reaches PC zero. Once a hot nonzero-PC frame proves it
            // cannot return to zero, the scheduler marks the decision closed
            // and hands it to the dormant interpreter path above.
            loop {
                let frame = self.vm.frame();
                let old_pc = frame.pc;
